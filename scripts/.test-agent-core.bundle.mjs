...
var buscarConPresupuesto = (input, ctx) => buscar({ presupuesto_max: 99e9, ...input }, ctx);
{
  const ctx = nuevoCtx();
...
}
{
  ...
  const ctx2 = { ...nuevoCtx([{ ...conTerraza, caracteristicas: ["Balc\xF3n"] }, sinNada]) };
  ...
  assert.equal(conBalcon.total, 1, '"balcon" tiene que encontrar "Balc\xF3n"');
...
}
{
  const enChico = (id, precio, area, hab) => ({
    ...
    barrio: "Los Rosales",
    ciudad: "Bogota",
    canon_arriendo: precio,
    area_m2: area,
    habitaciones: hab,
    banos: 2
  });
  const inventario = [
    enChico("barato", 42e5, 49, 1),
    enChico("bajo", 61e5, 90, 2),
    enChico("medio", 7e6, 120, 3),
    enChico("alto", 79e5, 160, 3),
    enChico("caro", 12e6, 200, 4)
  ];
  ...
  const ctx2 = nuevoCtx([enChico("rozando", 54e5, 100, 3), enChico("barato", 42e5, 49, 1)]);
